...
interface RecentInvoice {
  id: number; invoiceCode: string; patientName: string;
  amount: string; method: string; paidAt: string; createdAt: string;
  appointment: { date: string; time: string } | null;
}

const METHOD_CFG: Record<string, { label: string; color: string; bg: string; icon: string }> = {
  cash:          { label: 'Tiền mặt',     color: 'text-green-700',  bg: 'bg-green-100',  icon: '💵' },
  qr_code:       { label: 'QR Code',      color: 'text-indigo-700', bg: 'bg-indigo-100', icon: '📱' },
  bank_transfer: { label: 'Chuyển khoản', color: 'text-blue-700',   bg: 'bg-blue-100',   icon: '🏦' },
  card:          { label: 'Thẻ',          color: 'text-amber-700',  bg: 'bg-amber-100',  icon: '💳' },
};
...
// Backend chỉ trả "month" dạng "yyyy-MM" (không có sẵn nhãn).
// Tự suy ra nhãn ngắn cho trục biểu đồ, ví dụ "2026-06" -> "T6".
  ...
  useEffect(() => {
    Promise.allSettled([
      ...
      if (invRes.status === 'fulfilled') {
        ...
        setInvoices(Array.isArray(d) ? d.slice(0, 10) : []);
      }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {/* Filter tháng/năm để chọn kỳ xuất báo cáo */}
          <select
            value={filterMonth}
          ...
          <button
            ...
            disabled={!!exporting}
            className="flex items-center gap-1.5 bg-red-600 hover:bg-red-700 disabled:opacity-50
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {summaryCards.map(c => (
              <div key={c.label} className={`rounded-2xl border border-gray-100 shadow-sm p-5 ${c.bg}`}>
                ...
                <div className={`text-2xl font-bold ${c.color}`}>{fmt(c.value)}</div>
              </div>
            ))}
          </div>
          ...
          <div className="grid lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                  ...
                  return (
                    <div key={m.month} className="flex-1 flex flex-col items-center gap-2 group">
                      ...
                      <div className="w-full flex items-end" style={{ height: '140px' }}>
                        <div
                          className={`w-full rounded-t-lg transition-all duration-500 ${isCurrentMonth ? 'bg-[#1a3a5c]' : 'bg-[#1a3a5c]/30 group-hover:bg-[#1a3a5c]/60'}`}
                          style={{ height: pct > 0 ? `${Math.max(pct, 4)}%` : '4px' }}
                          title={fmt(m.revenue)}
                        />
                      </div>
                      {/* Label */}
                      <div className={`text-xs font-semibold ${isCurrentMonth ? 'text-[#1a3a5c]' : 'text-gray-400'}`}>
                        {monthLabel(m.month)}
                      </div>
                    </div>
                  );
              ...
              {maxRevenue === 1 && (
                <p className="text-xs text-gray-400 text-center mt-4">Chưa có dữ liệu doanh thu</p>
              )}
            </div>
              ...
              <div className="space-y-4">
                {Object.entries(stats?.byMethod ?? {}).length === 0 ? (
                  ...
                  Object.entries(stats?.byMethod ?? {})
                    .sort(([, a], [, b]) => b - a)
                    .map(([method, total]) => {
                      const cfg = METHOD_CFG[method] ?? { label: method, color: 'text-gray-600', bg: 'bg-gray-100', icon: '💰' };
                      ...
                      return (
                        <div key={method}>
                          <div className="flex items-center justify-between mb-1.5">
                            ...
                            <span className="text-xs font-bold text-gray-600">{fmt(total)}</span>
                          </div>
                          <div className="w-full bg-gray-100 rounded-full h-2">
                            <div
                              className="h-2 rounded-full bg-[#1a3a5c] transition-all duration-700"
                              style={{ width: `${pct}%` }}
                            />
                          </div>
                      ...
                      return (
                        <tr key={inv.id} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="px-4 py-3 hidden md:table-cell">
                            {m ? (
                              <span className={`text-xs px-2 py-0.5 rounded-full font-bold ${m.bg} ${m.color}`}>
                                {m.icon} {m.label}
                              </span>
                            ) : inv.method}
                          </td>
