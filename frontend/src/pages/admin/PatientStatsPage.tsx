    ...
    <div className="space-y-6">
          ...
          {data.visitsByMonth.map(mv => (
            <div key={mv.month} className="flex-1 flex flex-col items-center gap-1">
              ...
              <div
                ...
                style={{ height: `${(mv.count / maxVisit) * 100}%`, minHeight: mv.count > 0 ? '4px' : '2px' }}
              />
              <span className="text-xs text-gray-400 whitespace-nowrap">
                {mv.month.slice(5)}/{mv.month.slice(0,4)}
              </span>
            </div>
          ))}
        </div>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="w-full bg-gray-100 rounded-full h-2">
                        <div
                          ...
                          style={{ width: `${(d.count / maxDoc) * 100}%` }}
                        />
                      </div>
                    </div>
                  </div>
          ...
          <div className="space-y-2">
            {Object.entries(data.appointmentStatusDist).map(([status, count]) => (
              <div key={status} className="flex items-center gap-3">
                <span className={`text-xs px-2 py-0.5 rounded-full font-medium w-32 text-center
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        ...
        {data.topDiagnoses.length === 0
          ? <p className="text-sm text-gray-400">Chưa có hồ sơ bệnh án nào có chẩn đoán.</p>
          : (
...
function StatCard({ icon, label, value, color }: {
  ...
  const textMap = {
    ...
    purple: 'text-purple-700', red: 'text-red-700',
  };
  return (
